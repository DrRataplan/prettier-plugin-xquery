...
declare function local:f ($ns as namespace-node()) as element() {
  <namespace
    ...
    is-node="{ $ns instance of node() }"
    is-untyped="{ data($ns) instance of xs:untypedAtomic }"
    local-name="{ local-name($ns) }"
    name="{ name($ns) }"
    namespace-uri="{ namespace-uri($ns) }"
    parent-exists="{ exists($ns/..) }"
    same-as-self="{ $ns is $ns }"
    string-value="{ string($ns) }"
    typed-value="{ data($ns) }" />
};
