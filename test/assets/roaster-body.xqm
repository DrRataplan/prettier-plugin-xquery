	...
	if (not(exists($request?config?requestBody?content))) then
		...
		if (not($request?config?requestBody?content instance of map(*))) then
			...
			(
				...
				let $media-type :=
					if (contains($raw-content-type-header-value, ";")) then
						substring-before($raw-content-type-header-value, ";")
					else
						$raw-content-type-header-value
				let $charset :=
					if (contains($raw-content-type-header-value, "charset=")) then
						(
						...
						map {
							...
							"format":
								if ($media-type = ("application/json", "text/json")) then
									...
									if ($media-type = ("application/xml", "text/xml", "image/svg+xml")) then
										"xml"
									else
										if ($media-type = ("multipart/form-data", "application/x-www-form-urlencoded")) then
											"form-data"
										else
											if (
...
declare %private function body:validate-value($schema as map(*)) as function(*) {
	...
	return function ($name as xs:string) as map(*) {
			if (not(map:contains($property-definitions, $name))) then
				...
				return if (not(exists($value)) and $is-required) then
						...
						if (count($value) > 1 and not($is-array)) then
							error(
								$errors:BAD_REQUEST,
