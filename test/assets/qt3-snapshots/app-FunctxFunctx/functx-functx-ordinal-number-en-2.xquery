...
declare function functx:ordinal-number-en ($num as xs:integer?) as xs:string {
  concat(
    xs:string($num),
    if (matches(xs:string($num), "[04-9]$|1[1-3]$")) then
      "th"
    else if (ends-with(xs:string($num), "1")) then
      "st"
    else if (ends-with(xs:string($num), "2")) then
      "nd"
    else if (ends-with(xs:string($num), "3")) then
      "rd"
    else
      ""
  )
