...
declare function functx:change-element-names-deep (
  ...
  if (count($oldNames) != count($newNames)) then
    error(xs:QName("functx:Different_number_of_names"))
  else
    for $node in $nodes
...
return let $in-xml-2 :=
    <in-xml xmlns:dty="http://datypic.com"> <a> <dty:b>b</dty:b> <c>c</c> </a> </in-xml>
  return (
      ...
      functx:change-element-names-deep(
        ...
        (xs:QName("x"), xs:QName("y"), xs:QName("z"))
      ),
      functx:change-element-names-deep(
        $in-xml-2,
        (xs:QName("dty:b"), xs:QName("c")),
        (xs:QName("q"), QName("http://new", "new:c"))
      )
    )
